...
options=optimset('Display','off','OutputFcn', @printX_perIter,'MaxIter',100000,'MaxFunEvals',100000,'TolFun',0.0001, 'Algorithm','trust-region-dogleg');
...
lam_opt = 0.001 * 11.4 / 0.426^2 * (1 + sin(linspace(0, T, t_step + 1)) / 2);
...
for i = 1 : t_step + 1
    % for j = 1 : x_step
        state(i) = integral_rep(x, dt * (i - 1), lam_opt(1:t_step), dt, t_step, L);
        % state(i)
        % state(j) = integral_rep(dx * j, 1, lam_opt(1:t_step), dt, t_step, L);
...
function derivative = Vv(k,x,t,lam,dt,t_step,tau)
    ...
    omega = k.^2 .* t + 1i * v_tilde .* k + lam_tilde;
    derivative = exp(1i * k * x - omega) .* initial_condition(k) ...
              - exp(1i * k * x ) .* ( exp(-omega) .* initial_condition(-k - 1i) ...
                                     + (2i * k - 1) .* B(k, lam, t, tau, dt, t_step, lam_tilde));
                                        % - 1i * Bnu(k,v,t,dt,t_step)));
    % state = -2i ./ k .* exp(1i * k * x); 
end
...
function matrix = necessary_stateub(var, x, dt, t_step, L, T)
    ...
    state_max(1:constraint_time) = 0.5; % sin(dt * (1:constraint_time)) * 0.4;
...
function state_val = integral_rep(x, t, lam, dt, t_step, L)
    state_val = integral(@(r) state_real(r,x, t,lam,dt,t_step),0, 10*L);%...
...
function state = V(k,x,t,lam,dt,t_step)
    ...
    state   = exp(1i * k * x - omega) .* initial_condition(k) ...
              - exp(1i * k .* x ) .* ( exp(-omega) .* initial_condition(-k - 1i) ...
                                     + (2i * k - 1) .* B(k,lam,t, t, dt,t_step, lam_tilde));
                                        % - 1i * Bnu(k,v,t,dt,t_step)));
    % state = -2i ./ k .* exp(1i * k * x); 
end

function g0 = B(k, lam, t, tau, dt, t_step, lam_tilde)
    % g0 = (1- exp(-k.^2 * t)) ./ k.^2;
    % len_k =  length(k);
    % g0 = zeros(1, len_k);
    % for i = 1 : len_k
    %     g0(i) = integral(@(tt) boundary_condition(tt) ...
    %                             .* exp(lam_int(tt,lam,dt,t_step) + k(i)^2 * (tt - t) ...
        ...
        for i = 1 : step
            ...
            g0 = g0 + bound * boundary_condition(tt) * dtt ...
                            * exp(lam_int(tt,lam,dt,t_step) - lam_tilde ...
                            + k.^2 .* (tt-t) + 1i * (vint(tt) - vint(t)) .* k);
        end
        g0 = g0 + (tau - tt) * (boundary_condition(tt) ...
                            * exp(lam_int(tt,lam,dt,t_step) - lam_tilde + k.^2 .* (tt-t) ...
                                  + 1i * (vint(tt) - vint(t)) .* k)...
